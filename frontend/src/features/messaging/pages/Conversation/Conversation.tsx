...
export function Conversation() {
  ...
  useEffect(() => {
    if (id == "new") {
      ...
      request<IConversation>({
        endpoint: `/api/v1/threads/conversations/${id}`,
        onSuccess: (data) => setConversation(data),
        onFailure: () => navigate("/threads"),
      });
    }
  }, [id, navigate]);
  ...
  async function addMessageToConversation(e: FormEvent<HTMLFormElement>) {
    ...
    await request<void>({
      endpoint: `/api/v1/threads/conversations/${conversation?.id}/messages`,
      method: "POST",
      body: JSON.stringify({
        receiverId:
  ...
  async function createConversationWithMessage(e: FormEvent<HTMLFormElement>) {
    ...
    await request<IConversation>({
      ...
      onSuccess: (conversation) => navigate(`/threads/conversations/${conversation.id}`),
      onFailure: (error) => console.log(error),
    });
  }
      ...
      {(conversation || creatingNewConversation) && (
        <>
          <div className={classes.header}>
            <button className={classes.back} onClick={() => navigate("/threads")}>
              {"<"}
            </button>
          </div>
          {conversation && (
                ...
                <div className={classes.suggestions}>
                    ...
                    .map((user) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (conversation) {
                            navigate(`/threads/conversations/${conversation.id}`);
                          } else {
                  ...
                  {suggestingUsers.length === 0 && (
                    <div style={{ padding: "1rem" }}>
                      You need allies in your Circle to start a thread.
                    </div>
                  )}
                </div>
              )}
